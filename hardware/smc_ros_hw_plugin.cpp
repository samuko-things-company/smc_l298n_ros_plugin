  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  for (const hardware_interface::ComponentInfo & joint : info_.joints)
  {
    ...
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("SMCDriverHardware"),
...
std::vector<hardware_interface::StateInterface> SMCDriverHardware::export_state_interfaces()
{
...
std::vector<hardware_interface::CommandInterface> SMCDriverHardware::export_command_interfaces()
{
...
hardware_interface::CallbackReturn SMCDriverHardware::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("SMCDriverHardware"), "Configuring ...please wait...");
  if (smc_.connected())
  {
    smc_.disconnect();
  }
  smc_.connect(cfg_.port, 115200, 100);
  RCLCPP_INFO(rclcpp::get_logger("SMCDriverHardware"), "Successfully configured!");

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn SMCDriverHardware::on_cleanup(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("SMCDriverHardware"), "Cleaning up ...please wait...");
  if (smc_.connected())
  {
    smc_.disconnect();
  }
  RCLCPP_INFO(rclcpp::get_logger("SMCDriverHardware"), "Successfully cleaned up!");

  return hardware_interface::CallbackReturn::SUCCESS;
}


hardware_interface::CallbackReturn SMCDriverHardware::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("SMCDriverHardware"), "Activating ...please wait...");
  if (!smc_.connected())
  {
    return hardware_interface::CallbackReturn::ERROR;
  }

  for (int i=1; i<=5; i+=1){ // wait for the smc to fully setup
    delay_ms(1000);
    RCLCPP_INFO(rclcpp::get_logger("SMCDriverHardware"), "configuring controller: %d sec" ,(i));
  }

  smc_.sendTargetVel(0.000, 0.000); // targetA, targetB
  RCLCPP_INFO(rclcpp::get_logger("SMCDriverHardware"), "configuration complete");

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn SMCDriverHardware::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("SMCDriverHardware"), "Deactivating ...please wait...");
  smc_.sendTargetVel(0.000, 0.000); // targetA, targetB
  RCLCPP_INFO(rclcpp::get_logger("SMCDriverHardware"), "Successfully deactivated!");

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::return_type SMCDriverHardware::read(
